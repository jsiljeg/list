...
| `modes.spec.mjs` | stepping bound to the *set* a card was opened from — search results, Filhov izbor, Najbolje ocijenjeni, Ikone and the sommelier's picks are five different sets and each has been wrong. Also the sommelier answering with a glass as well as a bottle (it searched `bottle-*` only, so it could offer nothing but a whole bottle), not repeating one wine across both blocks, and every dish having at least three wines that score. |
...
| `data.spec.mjs` (menu) | the kitchen's dishes against the wine vocabulary: every dish pairing and style reachable on some wine we pour (the Tiramisu asked for `coffee`, which was in no dictionary and on no wine, and scored zero in silence), and no pairing key invented for one bottle — three wines carried lists pasted from their producer's notes, unreadable on the card and unmatchable by any dish. |
| `localization.spec.mjs` (regions) | the Regions screen in Chinese down to the last map label: no appellation chip and no label drawn inside a map left in bare Latin, and no map label carrying the bracketed Latin form that has no room at a hand-picked x/y. Plus every region card having a name, a tagline and a blurb in every language. |
