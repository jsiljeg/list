...
| `data.spec.mjs` | the conventions in CLAUDE.md, checked instead of trusted: no country inside `insight.region`, blends name-first and descending, large-format twins identical, critic names from the agreed list, every data file in canonical form, and — since the library split — every listing's `ref` resolving to a library wine with nothing stranded. Runs without a browser, in about a second. |
